...
export const CurrentBetsPage: React.FC = () => {
  ...
  const summaryCards = [
    { label: 'Active Bets', value: activeBets, color: 'text-orange-400' },
    { label: 'Total Exposure', value: `₹${totalExposure.toLocaleString()}`, color: 'text-amber-400' },
    { label: 'Total Stake Points', value: `₹${totalStake.toLocaleString()}`, color: 'text-slate-100' }
  ];
  ...
  const columns: TableColumn<CurrentBetEntry>[] = [
    ...
    },
    {
      header: 'Exposure',
      key: 'exposure',
      renderCell: (row) => <span className="text-amber-400 font-bold font-mono">₹{row.exposure.toLocaleString()}</span>
    },
    { header: 'Placed At', key: 'placedAt', renderCell: (row) => <span className="text-slate-400 font-medium text-[11px] font-mono">{row.placedAt}</span> },
      ...
      <div className="bg-[#131B2E] border border-[#1E293B] rounded-[12px] p-4 flex flex-col gap-3 text-left text-xs shadow-md font-mono">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex justify-between items-center">
            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Selection / Type</span>
          ...
          <div className="flex justify-between items-center">
            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Exposure</span>
            <span className="text-amber-400 font-bold">₹{row.exposure.toLocaleString()}</span>
          </div>
        </div>
      </div>
    );

    ...
    <SuperAdminLayout>
      <div className="flex flex-col text-left select-none animate-fadeIn">
        ...
        <nav className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-5">
          ...
          <ChevronRight className="w-3 h-3 text-zinc-400" />
          <Link to="/admin/settings/add-match-list" className="hover:text-zinc-955 transition-colors">
            Add Match List
          </Link>
          <ChevronRight className="w-3 h-3 text-zinc-400" />
          <span className="text-zinc-955">{sport.name}</span>
        </nav>
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-zinc-550 mt-1.5 leading-relaxed">
            Configure system match schedules, markets listings, and limit settings specifically for
            {sport.name}.
          </p>
        </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-[8px] p-8 shadow-sm flex flex-col items-center justify-center min-h-[320px] text-center">
          ...
          <h2 className="text-sm font-semibold text-zinc-800">{sport.name} Matches</h2>

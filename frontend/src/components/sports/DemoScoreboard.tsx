...
export const DemoScoreboard: React.FC<DemoScoreboardProps> = ({ event }) => {
  ...
  if (event.status === 'Upcoming') {
    return <div className="text-[11px] text-slate-400 font-medium">Match starts: {event.startTime}</div>;
  }
      ...
      return (
        <div className="flex flex-col gap-1 text-left select-none">
          <div className="text-xs font-bold text-slate-100">
            {participants.home.name}: <span className="text-amber-400">{participants.home.score}/{participants.home.subScore}</span>
            ...
            {meta?.currentOverRuns && (
              <span className="flex items-center gap-1">
                ...
                {meta.currentOverRuns.map((r: any, idx: number) => (
                  <span
                    key={idx}
                    className="px-1.5 py-0.5 bg-[#0E1524] border border-[#233252] rounded text-[9px] font-bold text-orange-400"
                  >
                    {r === 6 ? '6' : r === 4 ? '4' : r}
                  </span>
                ))}
      ...
      return (
        <div className="flex items-center gap-4 text-left select-none">
          <div className="text-sm font-bold text-slate-100 font-mono">
            <span className="text-amber-400">{participants.home.score}</span> - <span className="text-amber-400">{participants.away.score}</span>
          </div>
          <div className="flex flex-col text-[10px] text-slate-400 font-mono leading-none">
      ...
      return (
        <div className="flex flex-col gap-0.5 text-left select-none">
          ...
          <div className="text-[10px] text-slate-400 font-mono">
            <span className="text-red-400 font-bold">{meta?.raidStatus || 'Raid Active'}</span>
    ...
    case 'golf':
      return (
        <div className="flex flex-col gap-1 text-left select-none">
          ...
          <div className="text-[10px] text-slate-400 font-mono">Hole: {meta?.hole || 1}</div>
        </div>
      );
          ...
          {meta?.runners && (
            <div className="flex flex-wrap gap-2 text-[10px] text-slate-400 font-mono">
              {meta.runners.slice(0, 3).map((r: any, idx: number) => (
                <span key={idx} className="bg-[#0E1524] px-2 py-0.5 border border-[#233252] rounded text-slate-200">
                  {idx + 1}. {r.name}
                </span>
              ))}
            </div>
    ...
    case 'e-games':
      return (
        <div className="flex flex-col gap-0.5 text-left select-none">
          ...
          <div className="text-[10px] text-slate-400 font-mono">Map {meta?.map || 1}</div>
        </div>
      );
